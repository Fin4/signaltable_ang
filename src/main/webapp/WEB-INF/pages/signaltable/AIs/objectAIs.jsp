...
<div id="workspace">
    ...
    <table>
        <thead>
        <tr><th colspan="2">Аналоговые входы</th>
            <th class="tablemenu">
                <a href="<c:url value='/${processControlObject.name}-downloadExcel'/>" class="mbutton">excel</a>
                <a href="<c:url value='/${processControlObject.name}/ai/new'/>" class="mbutton">add new</a>
            </th>
        </tr>
        <tr>
            <th>ID</th>
        ...
        <c:forEach items="${analogInputs}" var="analogInput">
            <tr>
                <td>${analogInput.id} <a href="<c:url value='/${processControlObject.name}/ai/${analogInput.id}-edit'/>" class="button">edit</a>
                    <c:url var="deleteURL" value='/${processControlObject.name}/ai/${analogInput.id}-delete'/>
                    <form action="${deleteURL}" method="POST">
                        <input id="analogInput" name="analogInput" type="hidden" value="${analogInput.id}"/>
